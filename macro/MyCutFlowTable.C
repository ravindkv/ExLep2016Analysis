...
string makeCutFlowRowRatio(TFile *fData, string rowLableBkg, string rowLableRatio, bool isRatio, bool isMu, bool isEle){
  ...
  TH1F * hEventsST = (TH1F*)fST->Get(chName+"/base/totalYield");
...
void makeTable(ofstream & outFile, bool isMu, bool isEle){
  ...
  outFile<<"\\hline "<<endl; 
  outFile<<"\\hline "<<endl; 
  outFile<<" & "<<"Trigger & "<<"ControlSel & "<<" PreSel & "<<" ZTag & "<<" MET & "<<"b-veto \\\\"<<endl;
  outFile<<"Process & "<<"$N_{evt}(100\\times \\epsilon)$ & "<<"$N_{evt}(100\\times \\epsilon)$ & "<<"$N_{evt}(100\\times \\epsilon)$ & "<<"$N_{evt}(100\\times \\epsilon)$ & "<<"$N_{evt}(100\\times \\epsilon)$ & "<<"$N_{evt}(100\\times \\epsilon)$ \\\\"<<endl;
  outFile<<"\\hline "<<endl; 
  outFile<<"\\hline "<<endl; 
  if(isMu){
  ...
  outFile<<makeCutFlowRow(fST, "Single t", isMu, isEle)<<endl;
  outFile<<makeCutFlowRow(fWJ, "$W$ + jets", isMu, isEle)<<endl;
  outFile<<"\\hline "<<endl;
  if(isMu)outFile<<makeCutFlowRowRatio(fMuData, "All Bkg", "Data/Bkg", false, isMu,isEle)<<endl;
  if(isEle)outFile<<makeCutFlowRowRatio(fEleData, "All Bkg", "Data/Bkg", false,isMu,isEle)<<endl;
  outFile<<"\\hline "<<endl;
  if(isMu)outFile<<makeCutFlowRow(fMuData, "Data", isMu, isEle)<<endl;
  if(isEle)outFile<<makeCutFlowRow(fEleData, "Data", isMu, isEle)<<endl;
  outFile<<"\\hline "<<endl;
  if(isMu)outFile<<makeCutFlowRowRatio(fMuData,"All Bkg","Data/Bkg",true,isMu, isEle)<<endl;
  if(isEle)outFile<<makeCutFlowRowRatio(fEleData,"All Bkg","Data/Bkg",true,isMu, isEle)<<endl;
  outFile<<"\\hline "<<endl;   
